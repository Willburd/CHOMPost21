...
/datum/objective/consume/hunter/New(mincount,maxcount)
...
/datum/objective/consume/deephunger/New(mincount,maxcount)
	target_amount = rand(mincount,maxcount)
...
/datum/objective/chuinfestation/check_completion()
	...
	var/infestedchus = 0
	for(var/mob/living/simple_mob/vore/alienanimals/chu/C in GLOB.living_mob_list)
		if(C.stat != DEAD)
			infestedchus++
