/obj/machinery/reagent_refinery/reactor
	...
	desc = "A reinforced chamber for high temperature distillation. Can be connected to a pipe network to change the interior atmosphere. It will only output chemicals when it has reached 50% capacity."
...
/obj/machinery/reagent_refinery/reactor/process()
	...
	if(reagents.total_volume > (reagents.maximum_volume / 0.5))
...
/obj/machinery/reagent_refinery/reactor/examine(mob/user, infix, suffix)
	...
	. += "The internal temperature is [GM.temperature]k at [GM.return_pressure()]kpa"
