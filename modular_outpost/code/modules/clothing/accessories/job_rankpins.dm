...
/proc/station_rank_guide(var/rank_index)
	switch(rank_index)
		...
		if(RANK_BORG)
			return "As a station synthetic you are considered warrant officer-0. You have authority in your specific module's field, but only while your laws match the expected company laws. You are considered beneath the rank of warrant officer-1, but above enlisted-4. You are authorized to deny crew of lesser rank from accessing secure areas or tools, unless otherwise they are authorized to do so by a higher ranking member of the crew. If your laws are found to be incorrect your rank will be disregarded until they are corrected."
		...
		if(RANK_AI)
			return "As a station AI you are considered warrant officer-5 with specialization in station operation, law and procedure. While you must still respond to crew orders, an AI may deny access to secure locations or tools without authorization by higher ranking crew, and make judgements for crew safety as according to rank. If your laws are found to not be the company's expected laws your rank will be disregarded until they are corrected."
