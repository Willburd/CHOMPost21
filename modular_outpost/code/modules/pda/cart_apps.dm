...
/datum/data/pda/app/prisoner_manager/update_ui(mob/user as mob, list/data)
	...
	for(var/obj/item/implant/tracking/B in GLOB.all_tracking_implants)
		...
		if(bl)
			...
			if(B.malfunction)
				implantData[++implantData.len] = list("x" = rand(1,300), "y" = rand(1,300), "z" = rand(1,300), "area" = "Unknown")
				continue
			if(!(bl.z in using_map.station_levels) || istype(bl, /turf/space))
				implantData[++implantData.len] = list("x" = "?", "y" = "?", "z" = "?", "area" = "Unknown")
				continue
			implantData[++implantData.len] = list("x" = bl.x, "y" = bl.y, "z" = bl.z, "area" = A.name)
