...
/datum/datacore/proc/manifest_modify(var/name, var/assignment, rank)
	ResetPDAManifest()
...
/datum/datacore/proc/CreateGeneralRecord(var/mob/living/carbon/human/H, var/id, hidden)
	ResetPDAManifest()
	var/icon/front
	var/icon/side
	if(H)
	...
	if(hidden)
		...
		general += G
		// SSjob.update_limit(JOB_ANOMALY, general.len) //CHOMPAdd // Outpost 21 edit - Outsider job removal

	return G

/datum/datacore/proc/CreateSecurityRecord(var/name, var/id, hidden)
	ResetPDAManifest()
...
/datum/datacore/proc/CreateMedicalRecord(var/name, var/id, hidden)
	ResetPDAManifest()
