/datum/antagonist/proc/add_antagonist(var/datum/mind/player, var/ignore_role, var/do_not_equip, var/move_to_spawn, var/do_not_announce, preserve_appearance)
...
/datum/antagonist/proc/add_antagonist_mind(var/datum/mind/player, var/ignore_role, var/nonstandard_role_type, nonstandard_role_msg)
	if(!istype(player))
		return 0
	if(!player.current)
		return 0
...
/datum/antagonist/proc/remove_antagonist(var/datum/mind/player, var/show_message, implanted)
