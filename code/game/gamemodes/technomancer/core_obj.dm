...
/obj/spellbutton/Initialize(mapload, var/path, var/new_name, new_icon_state)
	. = ..()
	src.core = loc
...
/obj/item/technomancer_core/proc/add_spell(var/path, var/new_name, ability_icon_state)
	if(!path || !ispath(path))
