...
/obj/item/inserted_spell/Initialize(mapload, var/user, obj/item/spell/insert/inserter)
	. = ..()
	host = loc
	origin = user
	if(light_color)
...
/obj/item/inserted_spell/proc/looped_insert(var/remaining_callbacks, mob/living/carbon/human/H)
	PROTECTED_RPOC(TRUE)
	return
