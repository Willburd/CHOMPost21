...
/obj/item/soulstone/proc/transfer_human(var/mob/living/carbon/human/T,mob/U)
	if(!istype(T))
		return;
...
/obj/item/soulstone/proc/transfer_shade(var/mob/living/simple_mob/construct/shade/T,mob/U)
	if(!istype(T))
		return;
	if (T.stat == DEAD)
...
/obj/item/soulstone/proc/transfer_construct(var/obj/structure/constructshell/T,mob/U)
...
/obj/item/soulstone/proc/transfer_soul(var/choice as text, var/target, mob/U as mob)
	switch(choice)
		if("VICTIM")
