...
/obj/machinery/bodyscanner/proc/go_out()
	...
	occupant.forceMove(src.loc) // was occupant.loc = src.loc, but that doesn't trigger exit(), and thus recursive radio listeners forwarded messages to the occupant as if they were still inside it for the rest of the round!
	occupant = null
	update_icon() //icon_state = "body_scanner_1" //VOREStation Edit - Health display for consoles with light and such.
	SStgui.update_uis(src)
	return
