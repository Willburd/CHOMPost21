...
/datum/pipe_recipe/pipe/New(var/label, obj/machinery/atmospherics/path)
	name = label
	pipe_type = path
...
/datum/pipe_recipe/disposal/New(var/label, var/ptype, var/state, dt=PIPE_DIRECTIONAL, sort=0)
	name = label
	icon_state = state
	pipe_type = ptype
	dirtype = dt
