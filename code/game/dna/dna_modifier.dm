...
/obj/machinery/dna_scannernew/MouseDrop_T(var/mob/target, mob/user) //Allows borgs to clone people without external assistance
	var/mob/living/carbon/WC = get_occupant()
...
/obj/machinery/dna_scannernew/attackby(var/obj/item/item as obj, mob/user as mob)
...
/obj/machinery/computer/scan_consolenew/proc/setInjectorBlock(var/obj/item/dnainjector/I, var/blk, datum/transhuman/body_record/buffer) // Traitgenes Stores the entire body record
 ...
 */
/obj/machinery/computer/scan_consolenew/proc/print_sleeve(var/mob/user, datum/transhuman/body_record/active_br)
	//deleted record
	if(!istype(active_br))
...
/obj/machinery/computer/scan_consolenew/proc/do_irradiate(var/lock_state, block)
	var/mob/living/carbon/WC = connected?.get_occupant()
	irradiating = 0
...
/obj/machinery/computer/scan_consolenew/proc/do_transfer(var/lock_state, bufferId)
	irradiating = 0
	connected.locked = lock_state
