...
/datum/dna/proc/SetUIValue(var/block,var/value,defer=0)
	if (block<=0) return
	ASSERT(value>=0)
	ASSERT(value<=4095)
	UI[block]=value
...
/datum/dna/proc/SetUIValueRange(var/block,var/value,var/maxvalue,defer=0)
	if (block<=0) return
...
/datum/dna/proc/GetUIValueRange(var/block,maxvalue)
	if (block<=0) return 0
...
/datum/dna/proc/SetUIState(var/block,var/on,defer=0)
	if (block<=0) return
	var/val
	if(on)
		val=rand(2050,4095)
...
/datum/dna/proc/SetUIBlock(var/block,var/value,defer=0)
	if (block<=0) return
	return SetUIValue(block,hex2num(value),defer)

// Get a sub-block from a block.
/datum/dna/proc/GetUISubBlock(var/block,subBlock)
	return copytext(GetUIBlock(block),subBlock,subBlock+1)

// Do not use this unless you absolutely have to.
// Set a block from a hex string.  This is inefficient.  If you can, use SetUIValue().
// Used in DNA modifiers.
/datum/dna/proc/SetUISubBlock(var/block,var/subBlock, var/newSubBlock, defer=0)
	if (block<=0) return
...
/datum/dna/proc/SetSEValue(var/block,var/value,defer=0)
	if (block<=0) return
	ASSERT(value>=0)
	ASSERT(value<=4095)
	SE[block]=value
...
/datum/dna/proc/SetSEValueRange(var/block,var/value,maxvalue)
	if (block<=0) return
	ASSERT(maxvalue<=4095)
...
// Getter version of above.
/datum/dna/proc/GetSEValueRange(var/block,maxvalue)
	if (block<=0) return 0
...
/datum/dna/proc/SetSEState(var/block,var/on,defer=0)
	if (block<=0) return
	var/list/BOUNDS=GetDNABounds(block)
	var/val
	if(on)
...
/datum/dna/proc/GetSEActivationLevel(var/block,number_of_levels)
...
// Used in DNA modifiers.
/datum/dna/proc/SetSESubBlock(var/block,var/subBlock, var/newSubBlock, defer=0)
	if (block<=0) return
