...
/obj/effect/effect/water/proc/set_up(var/turf/target, var/step_count = 5, delay = 5)
	if(!target)
		return
	step_process(target, step_count, delay)

/obj/effect/effect/water/proc/step_process(var/turf/target, var/step_count, var/delay, iteration)
	step_count--
	if(!loc)
		qdel(src)
		return
