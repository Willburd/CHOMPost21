...
/proc/get_poster_decl(var/path = null, var/exact = TRUE, forbid_types)
	if(ispath(path))
		if(exact)
...
/obj/item/poster/afterattack(var/atom/A, var/mob/user, var/adjacent, clickparams)
	if(!adjacent)
		return FALSE
...
/obj/structure/sign/poster/Initialize(mapload, var/placement_dir = null, obj/item/poster/P = null)
	. = ..()
