/proc/get_poster_decl(var/path = null, var/exact = TRUE)
	if(ispath(path))
		if(exact)
			...
			var/list/L = decls_repository.get_decls_of_subtype(path) // Outpost 21 edit - Use get_decls_of_subtype instead of get_decls_of_type, or it will get the map placing icon_state
			return L[pick(L)]
	return null
