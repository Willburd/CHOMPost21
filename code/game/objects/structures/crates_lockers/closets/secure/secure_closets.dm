...
/obj/structure/closet/secure_closet/emag_act(var/remaining_charges, var/mob/user, var/emag_source, var/visual_feedback = "", audible_feedback = "")
	if(!broken)
		broken = 1
		locked = 0
...
/obj/structure/closet/secure_closet/mind/Initialize(mapload, var/datum/mind/mind_target, del_self = 1)
	. = ..()
	self_del = del_self
	if(mind_target)
		owner = mind_target
