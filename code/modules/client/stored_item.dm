...
/obj/machinery/item_bank/proc/start_using(mob/living/user)
	...
	if(!choice || choice == "Cancel" || !Adjacent(user) || inoperable() || panel_open)
		...
		*/
		...
		to_chat(user, span_notice("\The [src] can store a single item for you between shifts! Anyone can withdraw from the bank one time per shift. Some items are not able to be accepted by the bank."))
		// Outpost 21 edit end
		busy_bank = FALSE
		return
	else if(!I)
