...
/obj/machinery/computer/stockexchange/proc/sell_some_shares(var/datum/stock/S, mob/user)
	if (!user || !S)
		return
	var/li = logged_in
	if (!li)
...
/obj/machinery/computer/stockexchange/proc/buy_some_shares(var/datum/stock/S, mob/user)
	if (!user || !S)
		return
	var/li = logged_in
	if (!li)
...
/obj/machinery/computer/stockexchange/proc/do_borrowing_deal(var/datum/borrow/B, mob/user)
