...
/datum/decl/overmap_event_handler/proc/create_events(var/z_level, overmap_size, number_of_events)
...
/datum/decl/overmap_event_handler/proc/acquire_event_turfs(var/number_of_turfs, var/distance_from_origin, list/candidate_turfs, continuous = TRUE)
...
/datum/decl/overmap_event_handler/proc/get_random_neighbour(var/turf/origin_turf, var/list/candidate_turfs, continuous = TRUE, range)
	var/fitting_turfs
	if(continuous)
...
/datum/decl/overmap_event_handler/proc/is_event_active(var/ship, event_type, severity)
	if(!ship_events[ship])	return
	for(var/datum/event/E in ship_events[ship])
		if(E.type == event_type && E.severity == severity)
			return E

/datum/decl/overmap_event_handler/proc/on_turf_entered(var/turf/new_loc, obj/effect/overmap/visitable/ship/ship, old_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return
...
/datum/decl/overmap_event_handler/proc/on_turf_exited(var/turf/old_loc, obj/effect/overmap/visitable/ship/ship, new_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return
...
/datum/decl/overmap_event_handler/proc/is_event_included(var/list/hazards, obj/effect/overmap/event/E, equal_or_better)//this proc is only used so it can break out of 2 loops cleanly
