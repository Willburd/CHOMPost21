...
/datum/decl/overmap_event_handler/proc/create_events(var/z_level, var/overmap_size, number_of_events)
...
/datum/decl/overmap_event_handler/proc/acquire_event_turfs(var/number_of_turfs, var/distance_from_origin, var/list/candidate_turfs, continuous = TRUE)
...
/datum/decl/overmap_event_handler/proc/get_random_neighbour(var/turf/origin_turf, var/list/candidate_turfs, var/continuous = TRUE, range)
	var/fitting_turfs
	if(continuous)
...
/datum/decl/overmap_event_handler/proc/start_hazard(var/obj/effect/overmap/visitable/ship/ship, obj/effect/overmap/event/hazard)//make these accept both hazards or events
...
/datum/decl/overmap_event_handler/proc/stop_hazard(var/obj/effect/overmap/visitable/ship/ship, obj/effect/overmap/event/hazard)
	for(var/event_type in hazard.events)
...
/datum/decl/overmap_event_handler/proc/is_event_active(var/ship, var/event_type, severity)
	if(!ship_events[ship])	return
	for(var/datum/event/E in ship_events[ship])
		if(E.type == event_type && E.severity == severity)
			return E

/datum/decl/overmap_event_handler/proc/on_turf_entered(var/turf/new_loc, var/obj/effect/overmap/visitable/ship/ship, old_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return
...
/datum/decl/overmap_event_handler/proc/on_turf_exited(var/turf/old_loc, var/obj/effect/overmap/visitable/ship/ship, new_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return
...
/datum/decl/overmap_event_handler/proc/is_event_in_turf(var/datum/event/E, turf/T)
	for(var/obj/effect/overmap/event/hazard in hazard_by_turf[T])
		if((E in hazard.events) && E.severity == hazard.difficulty)
			return TRUE

/datum/decl/overmap_event_handler/proc/is_event_included(var/list/hazards, var/obj/effect/overmap/event/E, equal_or_better)//this proc is only used so it can break out of 2 loops cleanly
