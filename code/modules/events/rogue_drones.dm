...
/datum/event/rogue_drone/announce()
	var/msg
	var/rng = rand(1,5)
	//CHOMPStation Edit Start, Outpost 21 edit - Use system name instead
	switch(rng)
		if(1)
			msg = "A combat drone wing operating in orbit around [using_map.starsys_name] has failed to return from a anti-piracy sweep. If any are sighted, \
			approach with caution."
		if(2)
			msg = "Contact has been lost with a combat drone wing in [using_map.starsys_name] orbit. If any are sighted in the area, approach with \
			caution."
		if(3)
			msg = "Unidentified hackers have targeted a combat drone wing deployed within the [using_map.starsys_name] system. If any are sighted in the area, approach with caution."
		if(4)
