	...
	if(awaygate.calibrated)
		...
		if(dest)
			...
			if(isliving(M) && dest.abductor)
				var/mob/living/L = M
				if(L.get_nutrition() > 500)
					L.set_nutrition(500) //If the aim is to negate people overpreparing, then they shouldn't be able to stuff themselves full of food either.
