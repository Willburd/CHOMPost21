...
/obj/machinery/librarycomp/tgui_act(action, params)
	...
	switch(action)
		...
		if("arcane_checkout")
			...
			usr.visible_message(span_infoplain(span_bold("\The [usr]") + " stares at the blank screen for a few moments, [H.p_their()] expression frozen in fear. When [H.p_they()] finally awaken from it, [H.p_they()] look a lot older."), 2)
			screenstate = "home"
