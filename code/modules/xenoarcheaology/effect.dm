...
/datum/artifact_effect/proc/getDescription()
	...
	switch(trigger)
		...
		if(TRIGGER_METHANE) // Outpost 21 edit - Methane
			. += " Activation index involves "+ span_bold("precise local atmospheric conditions.") + " Methane is a potential trigger. (Atmosphere must be >[ARTIFACT_GAS_TRIGGER] MOL of gas to activate device)"
		else
