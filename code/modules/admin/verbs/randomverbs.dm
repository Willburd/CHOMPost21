...
ADMIN_VERB(cmd_admin_world_narrate, R_FUN|R_EVENT, "Global Narrate", "Globally narrate.", ADMIN_CATEGORY_FUN_NARRATE) // Allows administrators to fluff events a little easier -- TLE
	...
	message_admins(span_large(span_blue(span_bold("GlobalNarrate: [key_name_admin(user)] : [msg]<BR>")))) // Outpost 21 edit(port) - Wrap in span large for visibility
...
/client/proc/get_ghosts(var/notify = 0,what = 2)
