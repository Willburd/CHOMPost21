...
/proc/build_click(var/mob/user, buildmode, params, obj/object)
...
/proc/build_drag(var/client/user, buildmode, var/atom/fromatom, var/atom/toatom, var/atom/fromloc, var/atom/toloc, var/fromcontrol, tocontrol, params)
	var/obj/effect/bmode/buildholder/holder = null
	for(var/obj/effect/bmode/buildholder/H)
		if(H.cl == user)
			holder = H
...
/obj/effect/bmode/buildmode/proc/make_rectangle(var/turf/A, var/turf/B, var/turf/wall_type, var/turf/floor_type, var/area_enabled, area_name)
	if(!A || !B) // No coords
		return
...
/proc/create_buildmode_area(var/area_name, turf/origin)
...
/proc/detect_room_buildmode(var/turf/first, allowedAreas = AREA_SPACE)
	if(!istype(first))
		return
