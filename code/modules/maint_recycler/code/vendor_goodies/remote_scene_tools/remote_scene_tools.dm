...
/obj/item/remote_scene_tool/see_emote(var/mob/M as mob, var/text, emote_type)
	//to_world_log("emote: [text] from [M] to [linked]")
	if(M == getWearer())
		//we're the one doing the emote
		transmit_emote(src, text,emote_type)

/obj/item/remote_scene_tool/proc/transmit_emote(var/mob/M as mob, var/text, emote_type)
