...
/atom/movable/proc/fall()
	...
	if(isliving(src))
		...
		if(L.flying) //Some other checks are done in the wings_toggle proc
			...
			else if(L.get_nutrition() < 300 && L.get_nutrition() > 299.4) //290 would be risky, as metabolism could mess it up. Let's do 289.
