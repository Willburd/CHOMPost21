...
/obj/machinery/pump/proc/set_state(var/target, message = TRUE)
	if(target == on)
		return FALSE
...
/turf/simulated/floor/lava/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
	R.add_reagent(REAGENT_ID_MINERALIZEDFLUID, round(volume / 2, 0.1))


/turf/simulated/floor/water/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
...
/turf/simulated/floor/water/pool/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
	R.add_reagent(REAGENT_ID_CHLORINE, round(volume / 10, 0.1))

/turf/simulated/floor/water/deep/pool/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
	R.add_reagent(REAGENT_ID_CHLORINE, round(volume / 10, 0.1))

/turf/simulated/floor/water/contaminated/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
	R.add_reagent(REAGENT_ID_VATSTABILIZER, round(volume / 2, 0.1))

/turf/simulated/mineral/pump_reagents(var/datum/reagents/R, volume)
	. = ..()
	if(density)
		return
	if(!sand_dug)
