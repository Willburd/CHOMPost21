...
/datum/reagent/drugs/bliss/affect_blood(mob/living/carbon/M, var/alien, var/removed)
	...
	if(prob_proc == TRUE && prob(10) && isturf(M.loc) && !istype(M.loc, /turf/space) && M.canmove && !M.restrained() && !M.GetResting()) // CHOMPstation edit - Stop drug movement from forcing crawling
...
/datum/reagent/drugs/talum_quem/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	...
	M.druggy = max(M.druggy, drug_strength)
	if(prob(10) && prob_proc == TRUE && isturf(M.loc) && !istype(M.loc, /turf/space) && M.canmove && !M.restrained() && !M.GetResting()) // Outpost 21 edit - Resting stops drug movement
		step(M, pick(cardinal))
		prob_proc = FALSE
