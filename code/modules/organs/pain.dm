...
/mob/living/carbon/proc/custom_pain(message, power, force)
	...
	if(client?.prefs?.read_preference(/datum/preference/toggle/pain_frequency))
		...
		if(force || (message != last_pain_message) || (world.time >= next_pain_time))
			...
			if(prob(power / 10) && !isbelly(loc) && !src.client?.prefs?.read_preference(/datum/preference/toggle/hide_pain_scream)) // No pain noises inside bellies. // Outpost 21 edit - Hide automatic pain scream
				emote("pain")
			// CHOMPAdd End
