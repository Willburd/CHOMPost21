...
/mob/living/carbon/human/updatehealth()
	...
	if((can_feel_pain() || ((isSynthetic() && synth_cosmetic_pain))) && !(src.client?.prefs?.read_preference(/datum/preference/toggle/hide_pain_scream))) // Are we capable of feeling pain? // Outpost 21 edit - Hide automatic pain scream
...
/mob/living/carbon/human/adjustBruteLoss(var/amount,include_robo)
...
//'include_robo' only applies to healing, for legacy purposes, as all damage typically hurts both types of organs
/mob/living/carbon/human/adjustFireLoss(var/amount,include_robo)
...
/mob/living/carbon/human/proc/adjustBruteLossByPart(var/amount, var/organ_name, obj/damage_source = null)
	amount = amount*species.brute_mod
...
/mob/living/carbon/human/proc/adjustFireLossByPart(var/amount, var/organ_name, obj/damage_source = null)
	amount = amount*species.burn_mod
	if (organ_name in organs_by_name)
		var/obj/item/organ/external/O = get_organ(organ_name)
...
//It automatically updates damage overlays if necesary
//It automatically updates health status
/mob/living/carbon/human/take_organ_damage(var/brute, var/burn, var/sharp = FALSE, edge = FALSE)
...
//'include_robo' only applies to healing, for legacy purposes, as all damage typically hurts both types of organs
/mob/living/carbon/human/heal_overall_damage(var/brute, var/burn, include_robo)
	var/list/obj/item/organ/external/parts = get_damaged_organs(brute,burn)
...
/mob/living/carbon/human/take_overall_damage(var/brute, var/burn, var/sharp = FALSE, var/edge = FALSE, used_weapon = null)
	var/list/obj/item/organ/external/parts = get_damageable_organs()
	var/update = 0
	while(parts.len && (brute>0 || burn>0) )
		var/obj/item/organ/external/picked = pick(parts)
...
/mob/living/carbon/human/apply_damage(var/damage = 0, var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/sharp = FALSE, var/edge = FALSE, var/obj/used_weapon = null, projectile = FALSE)
