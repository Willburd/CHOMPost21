...
/mob/living/carbon/human/proc/add_marking(var/datum/sprite_accessory/marking/mark_datum, mark_color = "#000000")
	if (!mark_datum)
		return FALSE
	var/success = FALSE
	for(var/BP in mark_datum.body_parts)
...
/mob/living/carbon/human/proc/change_priority_of_marking(var/datum/sprite_accessory/marking/mark_datum, var/move_down, swap = TRUE) //move_down should be true/false
	if (!mark_datum)
		return FALSE
...
/mob/living/carbon/human/proc/change_priority_marking_to_priority(var/priority, to_priority)
	for (var/obj/item/organ/external/O in organs)
		for (var/marking in O.markings)
			if (O.markings[marking]["priority"] == priority)
				O.markings[marking]["priority"] = to_priority

/mob/living/carbon/human/proc/change_marking_color(var/datum/sprite_accessory/marking/mark_datum, mark_color = "#000000")
	if (!mark_datum)
		return FALSE
	var/success = FALSE
	for(var/BP in mark_datum.body_parts)
...
/mob/living/carbon/human/proc/transform_into_other_human(var/mob/living/carbon/human/character, var/copy_name, var/copy_flavour = TRUE, var/convert_to_prosthetics = FALSE, apply_bloodtype = TRUE)
	/*
