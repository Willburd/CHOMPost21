...
/datum/trait/negative/lonely/proc/handle_loneliness(var/mob/living/carbon/human/H)
	...
	if(H.loneliness_stage >= 250)
		...
		if(H.stuttering < hallucination_cap)
			H.stuttering += 5
	if(H.loneliness_stage >= warning_cap)
		ms = "<span class='danger'><b>[pick("Where are the others?", "Please, there has to be someone nearby!", "I don't want to be alone!","Please, anyone! I don't want to be alone!")]</b></span>" // Outpost 21 edit - More variety
	if(world.time < H.next_loneliness_time)
		return
	if(ms != "")
		to_chat(H, ms)
