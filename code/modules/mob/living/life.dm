/mob/living/Life()
	...
	if(stat != DEAD)
		...
		if(client)
			...
			if(client.inactivity_ISSUEHERE >= idle_limit && !away_from_keyboard && client.prefs?.read_preference(/datum/preference/toggle/auto_afk))	//if we're not already afk and we've been idle too long, and we have automarking enabled... then automark it
			...
			else if(away_from_keyboard && client.inactivity_ISSUEHERE < idle_limit && !manual_afk) //if we're afk but we do something AND we weren't manually flagged as afk, unmark it
...
/mob/living/proc/handle_light()
	...
	else if(glow_toggle && !is_ventcrawling) // Outpost 21 edit hiding in vents hides your lights too!
