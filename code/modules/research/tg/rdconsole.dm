...
/obj/machinery/computer/rdconsole_tg/tgui_data(mob/user)
	...
	for(var/v in stored_research.tiers)
		...
		if (!stored_research.researched_nodes[n.id] && length(n.announce_channels) && length(filter_department) && !(filter_department in n.announce_channels) && !(CHANNEL_COMMON in n.announce_channels))
			continue
		// Outpost 21 edit end
