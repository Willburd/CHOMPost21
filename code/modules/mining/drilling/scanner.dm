...
/obj/item/mining_scanner/proc/ScanTurf(var/atom/target, var/mob/user)
	...
	for(var/turf/simulated/T in range(range, Turf))
		...
		for(var/metal in T.resources)
			...
			switch(metal)
				if(ORE_SAND, ORE_CARBON, ORE_MARBLE, ORE_QUARTZ)				ore_type = "surface minerals" // Outpost 21 edit begin - Restored older ores
				if(ORE_HEMATITE, ORE_TIN, ORE_COPPER, ORE_BAUXITE, ORE_LEAD)	ore_type = "industrial metals" // Outpost 21 edit begin - Restored older ores
				if(ORE_GOLD, ORE_SILVER, ORE_RUTILE)							ore_type = "precious metals"
				if(ORE_DIAMOND, ORE_PAINITE)									ore_type = "precious gems" // Outpost 21 edit begin - Restored older ores
				if(ORE_URANIUM)													ore_type = "nuclear fuel"
				if(ORE_PHORON, ORE_PLATINUM, ORE_MHYDROGEN)						ore_type = "exotic matter"
				if(ORE_VERDANTIUM, ORE_VOPAL)									ore_type = "anomalous matter" // Outpost 21 edit begin - Restored older ores
