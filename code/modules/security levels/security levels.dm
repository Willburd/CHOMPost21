...
/proc/set_security_level(var/level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != GLOB.security_level)
		switch(level)
			...
			if(SEC_LEVEL_VIOLET)
				if(GLOB.security_level < SEC_LEVEL_VIOLET)
					security_announcement_up.Announce("[CONFIG_GET(string/alert_desc_violet_upto)]", "Attention! Alert level elevated to violet", new_sound = sound('sound/effects/alert_levels/red_alert.ogg')) // Outpost 21 edit
				else
			...
			if(SEC_LEVEL_ORANGE)
				if(GLOB.security_level < SEC_LEVEL_ORANGE)
					security_announcement_up.Announce("[CONFIG_GET(string/alert_desc_orange_upto)]", "Attention! Alert level elevated to orange", new_sound = sound('sound/effects/alert_levels/red_alert.ogg')) // Outpost 21 edit
				else
			...
			if(SEC_LEVEL_RED)
				if(GLOB.security_level < SEC_LEVEL_RED)
					security_announcement_up.Announce("[CONFIG_GET(string/alert_desc_red_upto)]", "Attention! Code red!", new_sound = sound('modular_outpost/sound/effects/alert_levels/outpost_red_alert.ogg', volume = 45)) // Outpost 21 edit
				else
		...
		spawn()
			SSnightshift.check_nightshift() //CHOMPedit: disabling this for now as we do not use the nightshift currently. // Outpost 21 edit - We do.
