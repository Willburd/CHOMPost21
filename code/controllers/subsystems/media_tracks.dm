...
/datum/controller/subsystem/media_tracks/proc/add_track(var/mob/user, var/new_url, var/new_title, var/new_duration, var/new_artist, var/new_genre, var/new_secret, new_lobby)
	if(!check_rights(R_DEBUG|R_FUN))
		return
...
/datum/controller/subsystem/media_tracks/proc/remove_track(var/mob/user, datum/track/T)
	if(!check_rights(R_DEBUG|R_FUN))
		return
