...
/datum/controller/subsystem/appreciation/proc/msg(var/message,sender)
	ASSERT(message)
...
/datum/controller/subsystem/appreciation/proc/cancel_appreciation(var/yes = 1,silent = FALSE)
	if(yes)
		if(!squelched && !silent)
