...
/datum/controller/subsystem/emergency_shuttle/proc/shuttle_arrived()
	...
	if(autopilot)
		...
		if(evac)
			...
			GLOB.priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_docked_message, "%dock_name%", "[using_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"), "Transfer System") // Outpost 21 edit - Removed virgo TTS , 'sound/AI/tramarrived.ogg') //VOREStation Edit - TTS
...
/datum/controller/subsystem/emergency_shuttle/proc/call_transfer()
	...
	GLOB.priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_called_message, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"), "Transfer System") // Outpost 21 edit, remove virgo TTS, 'sound/AI/tramcalled.ogg')
	SSatc.shift_ending()
