...
/datum/controller/subsystem/transfer/fire(resumed)
	...
	if (round_duration_in_ds >= shift_last_vote - 2 MINUTES)
		...
		to_chat(world, span_world(span_notice("Warning: This upcoming round-extend vote will be your ONLY extend vote. Wrap up your scenes in the next [hours] hours if the round is extended."))) // Outpost 21 edit(port) - calculate hours
