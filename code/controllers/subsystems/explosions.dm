...
/datum/controller/subsystem/explosions/proc/append_currentrun(var/x0,var/y0,var/z0,pwr,direction,starting_power)
	SHOULD_NOT_OVERRIDE(TRUE)
	PRIVATE_PROC(TRUE)
	if(pwr <= 0)
		return
...
/datum/controller/subsystem/explosions/proc/finalize_explosion(var/x0,var/y0,z0,pwr,max_starting)
	SHOULD_NOT_OVERRIDE(TRUE)
	PRIVATE_PROC(TRUE)
	if(pwr <= 0)
		return
