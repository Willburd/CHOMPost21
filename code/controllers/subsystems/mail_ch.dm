...
/datum/controller/subsystem/mail/proc/create_mail()
	...
	for(var/mob/living/carbon/human/player_human in player_list)
		if(player_human.stat != DEAD && player_human.client && player_human.client.inactivity <= 10 MINUTES && player_human.job != JOB_STOWAWAY && player_human.job != JOB_OUTSIDER && player_human.job != JOB_ANOMALY && !player_is_antag(player_human.mind)) // Only alive, active and NT employeers should be getting mail. Outpost 21 edit - Stowaways too
